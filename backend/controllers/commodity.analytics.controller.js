...
function computeStats(prices) {
  ...
  const stdDev = Math.sqrt(prices.reduce((s, v) => s + Math.pow(v - mean, 2), 0) / prices.length);
...
function buildInsight(name, stats, source) {
  if (!stats) return { text: `No data available for ${name}.`, signal: 'neutral' };
...
export const getCommodityMeta = async (req, res) => {
  try {
    ...
    const glCommodities = await tryMongo(() =>
      GlCommodity.find(filter)
        .sort({ commodity_name: 1 })
        .lean()
    );

    if (!glCommodities?.length) {
      return ok(res, [], { source: 'empty', message: 'No commodities found in greenleaf-dev.commodities' });
    }
    ...
    const KNOWN_MANDI_NAMES = new Set([
      'Cotton', 'Soybean', 'Wheat', 'Mustard', 'Onion', 'Turmeric',
      'Chana', 'Paddy', 'Tomato', 'Groundnut', 'Bajra',
    ]);
    ...
    const enriched = glCommodities.map(c => {
      ...
      const hasMandiSpread = KNOWN_MANDI_NAMES.has(c.parent_name) || KNOWN_MANDI_NAMES.has(c.commodity_name);
      ...
      return {
        ...
        source:        hasValues ? 'gl_values' : (hasMandiSpread ? 'agroindia_mandi' : 'price_only'),
      };
    });
...
export const getCommodityCompare = async (req, res) => {
  try {
    ...
    const names   = commodities.split(',').map(n => n.trim()).filter(Boolean).slice(0, 5);
    ...
    for (const name of names) {
      ...
      if (!data.length) {
        ...
        if (rows?.length) {
          data = rows.map(r => ({
            date:  r.priceDate?.toISOString?.()?.split('T')[0] || String(r.priceDate),
            price: r.modalPrice,
            min:   r.minPrice,
            max:   r.maxPrice,
          }));
    ...
    const bestToSell      = withStats.sort((a, b) => (b[1].stats.lastPrice || 0) - (a[1].stats.lastPrice || 0))[0]?.[0];
    const strongestTrend  = withStats.sort((a, b) => (b[1].stats.changePct || 0) - (a[1].stats.changePct || 0))[0]?.[0];
    const highestVolatility = withStats.sort((a, b) => (b[1].stats.volatility || 0) - (a[1].stats.volatility || 0))[0]?.[0];

    return ok(res, { series, summary: { bestToSell, strongestTrend, highestVolatility } }, {
      days: dayNum,
      commodities: names,
    });
  } catch (e) {
...
export const getCommodityFutures = async (req, res) => {
  try {
    ...
    if (!cid) {
      return ok(res, { contracts: [], message: `Commodity "${commodity}" not found in database.` }, {
        source: 'empty',
      });
    }
    ...
    const latest = await tryMongo(() =>
      GlCommodityFutures.findOne(filter)
        .sort({ date: -1 })
        .lean()
    );

    if (!latest) {
      return ok(res, { contracts: [], message: `No futures data available for ${commodity}.` }, {
        source: 'empty',
      });
    }
    ...
    if (latest.expiry_data?.length) {
      ...
      contracts = mapped.filter(c =>
        c.contract ||  // has a contract name, OR
        c.openPrice !== null || c.highPrice !== null || c.lowPrice !== null ||
        c.lastPrice !== null || c.volume !== null
      );
    ...
    const noDataMsg = contracts.length === 0
...
export const getCommoditySeasonality = async (req, res) => {
  try {
    ...
    let monthly = [], source = 'empty', dataYears = [], totalRecords = 0;
    ...
    if (cid) {
      ...
      if (rows?.length >= 10) {
        ...
        for (const r of rows) {
          ...
          if (!byMonth[month]) byMonth[month] = { prices: [], years: new Set() };
        ...
        monthly = Array.from({ length: 12 }, (_, i) => {
          ...
          if (!d || !d.prices.length) {
            return { month: m, monthName: MONTH_NAMES[i], avgPrice: null, minPrice: null, maxPrice: null, records: 0 };
          }
        ...
        const allYears = [...new Set(rows.map(r => r.date?.split('-')[0]).filter(Boolean))];
    ...
    if (source === 'empty') {
      ...
      if (rows?.length >= 10) {
        ...
        dataYears = [...new Set(rows.map(r => new Date(r.priceDate).getFullYear().toString()))].sort();
        source = 'agroindia_mandi';
      }
    }
    ...
    const noteText = source === 'gl_values'
...
export const getMandiSpread = async (req, res) => {
  try {
    ...
    const spreadRange = Math.round((best?.modalPrice || 0) - (worst?.modalPrice || 0));
