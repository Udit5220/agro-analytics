...
// 7-Day Outbreak Forecast generator
const generate7DayForecast = (baseScore, crop) => {
  const days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  const today = new Date().getDay();
  const forecast = [];
  const topPathogen = crop.toLowerCase().includes("rice") ? "Blast Disease" : crop.toLowerCase().includes("wheat") ? "Yellow Rust" : "Leaf Blight";

  for (let i = 0; i < 7; i++) {
    const dayName = days[(today + i) % 7];
    // create organic fluctuation
    const variance = Math.sin(i * 0.9) * 12 + (i * 1.5);
    ...
    forecast.push({
      ...
      score,
      level,
      threat: score > 40 ? topPathogen : "None Expected",
    });
  }
  return forecast;
};
...
export default function RiskPredictionEngine() {
  ...
  useEffect(() => {
    const loadFarms = async () => {
      try {
        ...
        setSelectedCrop(USER_PROFILE.farms[0].crops[0].name);
      }
    };
    loadFarms();
  }, []);
  ...
  const handleRunInference = useCallback(async () => {
    ...
    try {
      // Check treatment history to reduce risk
      let treatmentReduction = 0;
      try {
        const history = JSON.parse(localStorage.getItem("treatmentHistory") || "[]");
        const now = new Date();
        const recent = history.find(t => {
          const tCrop = t.crop || "";
          const tDate = new Date(t.date || t.timestamp);
          const diffDays = (now - tDate) / (1000 * 60 * 60 * 24);
          return tCrop.toLowerCase().includes(selectedCrop.toLowerCase()) && diffDays <= 7;
        });
        if (recent) {
          treatmentReduction = 25; // 25% risk reduction if a treatment was applied within 7 days
        }
      } catch (e) {
        console.error("Error reading treatment history in Risk Engine", e);
      }
      ...
      const adjustedScore = Math.max(0, riskResult.compositeRiskScore - treatmentReduction);
      const adjustedRiskLevel = adjustedScore >= 75 ? "High" : adjustedScore >= 45 ? "Medium" : "Low";

      const pathogens = generatePathogens(adjustedScore);
      ...
      if (treatmentReduction > 0) {
        analysis += ` (Risk score reduced by ${treatmentReduction}% due to recent crop treatment applied in the last 7 days).`;
      }

      // Update with local calculation
      setPredictionData({
        compositeRiskScore: adjustedScore,
      ...
      // On high risk calculations, generate a corresponding alert in diseaseAlerts
      if (adjustedScore >= 70) {
        try {
          const storedAlerts = localStorage.getItem("diseaseAlerts") || "[]";
          const list = JSON.parse(storedAlerts);
          const topPathogen = pathogens[0]?.name || "Disease Outbreak Risk";
          // Check if alert already exists for this crop & pathogen
          const alertExists = list.some(a => a.crop === selectedCrop && a.disease === topPathogen && !a.resolved);
          if (!alertExists) {
            const newAlert = {
              id: `alert-${Date.now()}`,
              crop: selectedCrop,
              ...
              resolved: false,
              symptoms: "High microclimatic wetness trigger factors met.",
            };
            list.unshift(newAlert);
            localStorage.setItem("diseaseAlerts", JSON.stringify(list));
      ...
      // Write to localStorage.heatmapRiskUpdates when manual scenarios run
      try {
        const heatmapRiskUpdates = JSON.parse(localStorage.getItem("heatmapRiskUpdates") || "{}");
        const districtName = location.district || "Faridabad";
        heatmapRiskUpdates[districtName] = {
          riskScore: adjustedScore,
          riskLevel: adjustedRiskLevel,
          crop: selectedCrop,
          timestamp: Date.now(),
        };
        localStorage.setItem("heatmapRiskUpdates", JSON.stringify(heatmapRiskUpdates));
        window.dispatchEvent(new Event("storage"));
      } catch (e) {
        console.error(e);
      }
      ...
      try {
        ...
        if (aiData && aiData.compositeRiskScore) {
          const aiAdjustedScore = Math.max(0, aiData.compositeRiskScore - treatmentReduction);
          const aiAdjustedLevel = aiAdjustedScore >= 75 ? "High" : aiAdjustedScore >= 45 ? "Medium" : "Low";
          let aiAnalysis = aiData.analysis || analysis;
          if (treatmentReduction > 0 && !aiAnalysis.includes("Risk score reduced")) {
            aiAnalysis += ` (Risk score reduced by ${treatmentReduction}% due to recent crop treatment applied in the last 7 days).`;
          }

          setPredictionData({
            compositeRiskScore: aiAdjustedScore,
            riskLevel: aiAdjustedLevel,
            pathogens: aiData.pathogens || pathogens,
            treatments: aiData.treatments || treatments,
            analysis: aiAnalysis,
          });

          // Generate alert if AI score is also high
          if (aiAdjustedScore >= 70) {
            const storedAlerts = localStorage.getItem("diseaseAlerts") || "[]";
            const list = JSON.parse(storedAlerts);
            const topPathogen = (aiData.pathogens && aiData.pathogens[0]?.name) || pathogens[0]?.name || "Disease Outbreak Risk";
            const alertExists = list.some(a => a.crop === selectedCrop && a.disease === topPathogen && !a.resolved);
            if (!alertExists) {
              const newAlert = {
                id: `alert-${Date.now()}`,
                crop: selectedCrop,
                disease: topPathogen,
                severity: aiAdjustedScore >= 85 ? "Critical" : "High",
                message: `AI Risk Prediction Engine detected high vulnerability (${aiAdjustedScore}% risk score) for ${selectedCrop}.`,
                source: "AI Forecast",
                timestamp: "Today",
                date: new Date().toISOString().split("T")[0],
                resolved: false,
          ...
          // Update heatmap updates too
          const heatmapRiskUpdates = JSON.parse(localStorage.getItem("heatmapRiskUpdates") || "{}");
          const districtName = location.district || "Faridabad";
          heatmapRiskUpdates[districtName] = {
            riskScore: aiAdjustedScore,
            riskLevel: aiAdjustedLevel,
            crop: selectedCrop,
            timestamp: Date.now(),
          };
  ...
  return (
    <div className="space-y-6 animate-fadeIn antialiased max-w-7xl mx-auto p-4">
      {/* Header */}
      <header className="border-b border-gray-200 pb-4 text-left">
        <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider block mb-1">
          Predictive Diagnostics
        </span>
        <h1 className="text-xl md:text-2xl font-bold tracking-tight text-[#132a13]">
        ...
        <div className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none bg-[#f4f7f4] border border-[#cbdcd5] rounded-xl px-4 py-2.5 text-xs font-bold text-gray-800 focus:outline-none focus:border-[#31572c] cursor-pointer"
                >
            ...
            <button
              onClick={handleRunInference}
              disabled={isLoading}
              className="w-full mt-5 py-3 bg-gradient-to-r from-[#31572c] to-[#132a13] hover:from-[#132a13] hover:to-[#0a1a0a] disabled:opacity-70 disabled:cursor-not-allowed text-white font-bold text-sm uppercase tracking-wider rounded-xl transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer"
            >
              {isLoading ? (
      ...
      {/* 7-Day Forecast, Spray Window & Triggers */}
      {hasRunDiagnostics && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6 animate-slideUp text-left">
          
          {/* CARD 1: 7-Day Outbreak Forecast */}
          <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-4">
            <h3 className="text-xs font-bold text-[#31572c] uppercase tracking-wider flex items-center gap-1.5 pb-2 border-b border-gray-100">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
              <span>7-Day Pathogen Projections</span>
            </h3>
            
            <div className="space-y-3">
              {generate7DayForecast(predictionData.compositeRiskScore, selectedCrop).map((f, i) => (
                <div key={i} className="flex items-center justify-between text-xs py-1 border-b border-gray-50/50 last:border-0">
                  <span className="font-semibold text-gray-700 w-12">{f.day}</span>
                  <div className="flex-1 mx-3 flex items-center gap-2">
                    <div className="flex-1 bg-gray-100 rounded-full h-1.5 overflow-hidden">
                      <div 
                        className={`h-full rounded-full ${f.score >= 75 ? 'bg-red-500' : f.score >= 45 ? 'bg-amber-500' : 'bg-emerald-500'}`} 
                        style={{ width: `${f.score}%` }}
                      ></div>
                    </div>
                    <span className="font-extrabold text-gray-800 w-8 text-right">{f.score}%</span>
                  </div>
                  <span className={`text-[8px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded border ${
                    f.level === 'High' ? 'bg-red-50 text-red-700 border-red-100' : 
                    f.level === 'Medium' ? 'bg-amber-50 text-amber-700 border-amber-100' : 
            ...
            {/* 30-Day Seasonal Forecast */}
            <div className="bg-[#f4f7f4] border border-[#31572c]/10 rounded-xl p-3 mt-3">
              <span className="text-[9px] font-black text-[#31572c] uppercase tracking-widest block mb-1">
                30-Day Seasonal Projections
              </span>
              <p className="text-[11px] font-bold text-gray-900 leading-relaxed font-sans">
                Elevated risk peaks around vegetative transition stage due to projected microclimatic wetness. Recommend soil nitrogen buffering.
              </p>
            </div>
          </div>

          {/* CARD 2: Best Preventive Spray Window */}
          <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-4">
            <h3 className="text-xs font-bold text-[#31572c] uppercase tracking-wider flex items-center gap-1.5 pb-2 border-b border-gray-100">
              <span className="w-2 h-2 rounded-full bg-blue-500"></span>
              <span>Preventive Spray Window</span>
            </h3>

            <div className="bg-gradient-to-br from-[#31572c]/10 to-[#ecf39e]/10 border border-[#31572c]/20 rounded-2xl p-4 text-center space-y-2">
              <span className="text-[8px] font-black text-[#132a13] uppercase tracking-widest bg-white border px-2.5 py-1 rounded-full shadow-sm">
            ...
            <div className="space-y-3">
              <span className="text-[9px] font-black text-gray-500 uppercase block tracking-wider">
                Suitability Telemetry
              </span>
              <div className="grid grid-cols-3 gap-2">
                <div className="bg-gray-50 border rounded-xl p-2 text-center">
                  <span className="text-[8px] font-bold text-gray-400 block uppercase">Wind Drift</span>
                  <span className="text-xs font-black text-emerald-700">Safe (6 km/h)</span>
                </div>
                <div className="bg-gray-50 border rounded-xl p-2 text-center">
                  <span className="text-[8px] font-bold text-gray-400 block uppercase">Rain Risk</span>
                  <span className="text-xs font-black text-emerald-700">Low (8%)</span>
                </div>
                <div className="bg-gray-50 border rounded-xl p-2 text-center">
                  <span className="text-[8px] font-bold text-gray-400 block uppercase">Evap Risk</span>
                  <span className="text-xs font-black text-emerald-700">Moderate</span>
                </div>
              </div>
            </div>

            {/* Quick reminder scheduler redirect */}
            <div className="pt-2">
              <a 
                href="/module/disease-detection/treat" 
                className="w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 font-extrabold text-[10px] uppercase tracking-wider rounded-xl transition-all block text-center border border-slate-200"
              >
                Schedule Spray Reminder in Advisor
              </a>
            </div>
          </div>

          {/* CARD 3: Weather-Disease Correlation & Disease Trigger Factors */}
          <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-4">
            <h3 className="text-xs font-bold text-[#31572c] uppercase tracking-wider flex items-center gap-1.5 pb-2 border-b border-gray-100">
              <span className="w-2 h-2 rounded-full bg-amber-500"></span>
              <span>Weather-Disease Correlation</span>
            </h3>
            ...
            <div className="space-y-3">
              ...
              {/* Relative factor percentages */}
              <div className="space-y-2">
                <div>
                  <div className="flex justify-between text-[10px] font-semibold text-gray-700 mb-0.5">
                    <span>Humidity ({humidity}%)</span>
                    <span className="text-red-600 font-bold">+{Math.round((humidity/100)*30)}% Risk</span>
                  </div>
                  <div className="bg-gray-100 rounded-full h-1 overflow-hidden">
                    <div className="bg-red-500 h-full" style={{ width: `${(humidity/100)*100}%` }}></div>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-[10px] font-semibold text-gray-700 mb-0.5">
                    ...
                    <span className="text-amber-600 font-bold font-sans">+{Math.round(Math.min(rainfall/100, 1)*25)}% Risk</span>
                  </div>
                  <div className="bg-gray-100 rounded-full h-1 overflow-hidden">
                    <div className="bg-amber-500 h-full" style={{ width: `${Math.min(rainfall/100, 1)*100}%` }}></div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="bg-gray-100 rounded-full h-1 overflow-hidden">
                    <div className="bg-emerald-500 h-full" style={{ width: `${(Math.max(0, 20 - Math.abs(optimalTemp - temperature)*1.2)/20)*100}%` }}></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2 border-t border-gray-100 pt-3">
              <span className="text-[9px] font-black text-gray-500 uppercase block tracking-wider">
                Microclimatic Triggers
              </span>
              <ul className="space-y-1.5 text-[11px] font-semibold text-gray-700">
                <li className="flex items-center gap-1.5 text-red-800">
                  <span className="w-1 h-1 rounded-full bg-red-600"></span>
                  <span>{windSpeed < 5 ? "Low Wind Speed (+10% spore density)" : "Optimal Wind Dispersion"}</span>
                </li>
                <li className="flex items-center gap-1.5 text-amber-800">
                  <span className="w-1 h-1 rounded-full bg-amber-600"></span>
                  <span>{growthStage === 'Vegetative' || growthStage === 'Flowering' ? `Vulnerable Growth Stage (${growthStage})` : "Low Stage Vulnerability"}</span>
                </li>
                <li className="flex items-center gap-1.5 text-blue-800">
                  <span className="w-1 h-1 rounded-full bg-blue-600"></span>
                  <span>Clay/Alluvial Soil moisture retention active</span>
                </li>
              </ul>
            </div>
          </div>
