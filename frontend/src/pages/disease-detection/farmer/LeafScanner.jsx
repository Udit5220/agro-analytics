...
export default function LeafScanner() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      // Save scan to localStorage.recentScans
      const storedScans = localStorage.getItem("recentScans") || "[]";
      try {
        const list = JSON.parse(storedScans);
        const newScan = {
          ...result,
          id: `scan-${Date.now()}`,
          date: new Date().toISOString().split("T")[0],
          severity: result.severity || "Moderate",
          recovery_probability: result.recovery_probability || "Medium",
        };
        list.unshift(newScan);
        localStorage.setItem("recentScans", JSON.stringify(list.slice(0, 10)));
      } catch (e) {
        console.error(e);
      }

      // Save alert to localStorage.diseaseAlerts
      const storedAlerts = localStorage.getItem("diseaseAlerts") || "[]";
      try {
        const list = JSON.parse(storedAlerts);
        const newAlert = {
          id: `alert-${Date.now()}`,
          crop: result.plant_name?.split("/")?.[0]?.trim() || "Crop",
          disease: result.disease_name || "Unknown Disease",
          severity: result.severity || "Moderate",
          message: `${result.disease_name} detected. Affected area: ${result.severity === "Critical" ? "78%" : result.severity === "High" ? "54%" : "27%"}. Symptoms: ${result.symptoms?.[0] || 'spots'}.`,
          source: "AI Scan",
          timestamp: "Today",
          date: new Date().toISOString().split("T")[0],
          resolved: false,
        ...
        <div className="bg-white border border-gray-300 rounded-3xl p-6 shadow-sm space-y-6 animate-slideUp text-left">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-gray-200">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="bg-white border rounded-xl px-3 py-1.5 text-center shadow-inner">
                <span className="text-[8px] text-gray-600 block uppercase tracking-wider font-bold">Area Affected</span>
                <span className="text-xs font-black text-red-600 font-sans">
                  {diagnosisReport.severity === "Critical" ? "78%" : diagnosisReport.severity === "High" ? "54%" : "27%"}
                </span>
              </div>
              <div className="bg-white border rounded-xl px-3 py-1.5 text-center shadow-inner">
                <span className="text-[8px] text-gray-600 block uppercase tracking-wider font-bold">Stage</span>
                <span className="text-xs font-black text-slate-800 font-sans">
                  {diagnosisReport.severity === "Critical" ? "Late Stage" : diagnosisReport.severity === "High" ? "Mid-to-Late Stage" : "Early Stage"}
                </span>
              </div>
              <div className="bg-white border rounded-xl px-3 py-1.5 text-center shadow-inner">
                <span className="text-[8px] text-gray-600 block uppercase tracking-wider font-bold">Yield Loss</span>
                <span className="text-xs font-black text-[#9a3412] font-sans">
                  {diagnosisReport.severity === "Critical" ? "35%–60%" : diagnosisReport.severity === "High" ? "15%–30%" : "8%–15%"}
                </span>
              </div>
              <div className="bg-white border rounded-xl px-3 py-1.5 text-center shadow-inner">
                <span className="text-[8px] text-gray-600 block uppercase tracking-wider font-bold">Re-Scan In</span>
                <span className="text-xs font-black text-blue-600 font-sans">
                  {diagnosisReport.severity === "Critical" ? "2 Days" : diagnosisReport.severity === "High" ? "3 Days" : "5 Days"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-5">
              ...
              <div className="bg-blue-50/20 border border-blue-100 rounded-2xl p-4 text-xs font-medium text-slate-700 leading-relaxed space-y-1">
                <span className="font-black text-slate-900 uppercase tracking-wider block">AI Diagnostic Confidence Explanation:</span>
                <p>
                  {diagnosisReport.confidence_explanation || 
                    `The AgroIndia pathology engine analyzed spot shape density, halo margins, and leaf color variance to conclude a ${diagnosisReport.confidence || '90%'} confidence match. The symptoms are characteristic of ${diagnosisReport.disease_name} during high-humidity crop lifecycle stages.`}
                </p>
              </div>
            ...
            <div className="space-y-5">
              ...
              {/* Before vs After Comparison Slider/Toggle */}
              <div className="bg-white border border-gray-300 rounded-2xl p-5 space-y-3">
                <h5 className="text-xs font-black text-gray-950 uppercase tracking-wider flex items-center gap-1.5">
                  <Activity className="w-4 h-4 text-[#31572c]" />
                  Before vs After Comparison
                </h5>
                <div className="flex items-center justify-center p-2 bg-[#f4f7f4]/40 rounded-xl relative overflow-hidden h-[180px] w-full border">
                  {showAfter ? (
                    <div className="w-24 h-28 animate-fadeIn flex flex-col items-center justify-center">
                      <svg viewBox="0 0 120 160" className="w-full h-full">
                        <path d="M60,10 C80,50 90,90 90,130 C90,150 75,150 60,150 C45,150 30,150 30,130 C30,90 40,50 60,10 Z" fill="#1b4332" />
                        <path d="M60,10 L60,150" stroke="#40916c" strokeWidth="2" />
                      </svg>
                      <span className="text-[8px] text-emerald-700 font-black mt-1 uppercase">Healthy Leaf</span>
                    </div>
                  ) : (
                    <div className="w-24 h-28 animate-fadeIn flex flex-col items-center justify-center">
                      {imagePreviewUrl ? (
                        <img src={imagePreviewUrl} alt="Infected leaf preview" className="w-full h-full object-cover rounded" />
                      ) : (
                ...
                <div className="flex justify-between items-center gap-2">
                  <button
                    ...
                    className={`flex-1 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition ${!showAfter ? 'bg-red-655 text-white shadow-sm' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                  >
                    Infected (Before)
                  </button>
                  <button
                    ...
                    className={`flex-1 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition ${showAfter ? 'bg-emerald-600 text-white shadow-sm' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                  >
                    Recovered (After)
                  </button>
                </div>
