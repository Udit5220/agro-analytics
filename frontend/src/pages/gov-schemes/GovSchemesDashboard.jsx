...
import { FileText, Award, Landmark, CircleDollarSign, ArrowUpRight, CheckCircle2, AlertCircle } from "lucide-react";

export default function GovSchemesDashboard() {
  const metrics = [
    { label: "Active Schemes", value: "214", sub: "Nationally matched", color: "text-emerald-700 bg-emerald-50" },
    { label: "Eligible Subsidies", value: "8 Schemes", sub: "High match score", color: "text-[#31572c] bg-[#31572c]/10" },
    { label: "Pending Claims", value: "2", sub: "Under verification", color: "text-sky-700 bg-sky-50" },
    { label: "Total Disbursed", value: "₹45,000", sub: "Platform verified", color: "text-amber-700 bg-amber-50" },
  ];

  const schemes = [
    { name: "PM-KISAN Samman Nidhi", match: "100% Eligible", payout: "₹6,000 / year", status: "Active Disbursal" },
    { name: "Agricultural Machinery Subsidy", match: "95% Eligible", payout: "50% Off Tractor/Seeder", status: "Apply Now" },
    { name: "PM Fasal Bima Yojana (PMFBY)", match: "92% Eligible", payout: "Crop Insurance Guard", status: "Active Cover" },
    { name: "Har Khet Ko Pani (PMKSY)", match: "88% Eligible", payout: "80% Tube-well Subsidy", status: "Verified" },
  ];
              ...
              <tbody className="divide-y divide-gray-100/60">
                {schemes.map((item, idx) => (
                  <tr key={idx} className="hover:bg-[#f4f7f4]/30 transition-colors text-xs font-semibold">
                    ...
                    <td className="p-3 text-emerald-700 font-black">{item.match}</td>
                    <td className="p-3 text-gray-550">{item.payout}</td>
                    <td className="p-3 text-right pr-2">
                      <span className={`inline-block text-[10px] font-bold px-2 py-0.5 rounded ${
                        item.status === 'Apply Now' ? 'bg-[#31572c] text-white cursor-pointer hover:bg-[#132a13]' : 'bg-emerald-50 text-emerald-700'
                      }`}>
                        {item.status}
                      </span>
                    </td>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-200/60 shadow-sm space-y-4">
          ...
          <div className="space-y-3">
            <div className="bg-amber-50/50 border border-amber-100 p-3 rounded-xl flex gap-2.5">
              ...
              <div>
                <span className="text-[10px] font-bold text-amber-900 block">e-KYC Mandatory Deadline</span>
            ...
            <div className="bg-emerald-50/50 border border-emerald-100 p-3 rounded-xl flex gap-2.5">
              ...
              <div>
                <span className="text-[10px] font-bold text-[#132a13] block">Super-Seeder Subsidy</span>
                <span className="text-[11px] text-gray-600 block mt-0.5 leading-relaxed font-medium">
                  Haryana Department of Agriculture opens online portal window. First-come first-served registry active.
                </span>
              </div>
            </div>
          </div>
