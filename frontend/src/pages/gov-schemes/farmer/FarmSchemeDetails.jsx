// src/pages/gov-schemes/farmer/FarmSchemeDetails.jsx
...
const FarmSchemeDetails = () => {
  ...
  // Fetch scheme details whenever selection changes
  useEffect(() => {
    if (!selectedScheme) return;
    setDetailsLoading(true);
    
    govSchemesApi.getSchemeById(selectedScheme)
      .then((res) => {
        if (res && res.scheme) {
          setCurrentScheme(res.scheme);
          // Log viewed telemetry interaction
          govSchemesApi.interact(selectedScheme, "view").catch(console.error);
        }
  ...
  if (loading) {
    return (
      <div className="p-6 text-center py-16 text-xs font-bold text-gray-500">
        Loading schemes guidance hub...
      </div>
    );
  }
  ...
  const details = currentScheme.details || {};
  const benefits = details.benefits || {};
  const eligibilityMatrix = details.eligibilityMatrix || [];
  const requiredDocs = details.documents || [];
  const timeline = details.timeline || { steps: [] };
  const faqList = details.faq || details.faqs || [];
  ...
  return (
    <div className="p-6 overflow-y-auto h-full bg-[#f4f7f4]/40 animate-fadeIn space-y-6">
      ...
      {detailsLoading ? (
        <div className="text-center py-12 text-xs font-bold text-gray-550">
          Fetching scheme guidelines...
        </div>
      ) : (
        <>
            ...
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div className="space-y-3">
                <span className="text-[10px] font-bold bg-[#132a13]/60 text-[#ecf39e] border border-brand-medium/30 px-3 py-1 rounded-full uppercase tracking-wider">
                  {details.ministry || "Ministry of Agriculture"}
                </span>
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  <span className="text-[10px] font-semibold bg-white/10 border border-white/20 px-2.5 py-0.5 rounded-lg text-white">
                    Launch Year: {details.launchYear || "N/A"}
                  </span>
                  <span className="text-[10px] font-semibold bg-white/10 border border-white/20 px-2.5 py-0.5 rounded-lg text-white">
                    Status: Active
                  </span>
              ...
              <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-xl p-4 md:text-right space-y-1.5 text-xs w-full md:w-auto shrink-0">
                ...
                <p className="font-bold text-sm text-[#ecf39e]">{details.target || "Farmers"}</p>
                <p className="text-white/70 pt-1">Budget Allocation:</p>
                <p className="font-bold text-white font-mono">{details.budget || "Central Subsidy"}</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white rounded-xl p-5 border border-gray-150 shadow-sm hover:shadow transition-all duration-200 flex flex-col justify-between min-h-[110px]">
              ...
              <p className="text-sm font-black text-gray-800 leading-snug mt-2">
                {benefits.maximumBenefit || currentScheme.benefit}
              </p>
            </div>
            ...
            <div className="bg-white rounded-xl p-5 border border-gray-150 shadow-sm hover:shadow transition-all duration-200 flex flex-col justify-between min-h-[110px]">
              ...
              <p className="text-sm font-black text-gray-800 leading-snug mt-2">
                {benefits.perInstallment || "Direct DBT Slabs"}
              </p>
            </div>
            ...
            <div className="bg-white rounded-xl p-5 border border-gray-150 shadow-sm hover:shadow transition-all duration-200 flex flex-col justify-between min-h-[110px]">
              ...
              <p className="text-sm font-black text-gray-800 leading-snug mt-2">
                {benefits.subsidyType || "Capital Subsidy"}
              </p>
            </div>
            ...
            <div className="bg-white rounded-xl p-5 border border-gray-150 shadow-sm hover:shadow transition-all duration-200 flex flex-col justify-between min-h-[110px]">
              ...
              <p className="text-sm font-black text-gray-800 leading-snug mt-2">
                {benefits.coverage || "Eligible farmers"}
              </p>
            </div>
          </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-150 overflow-hidden">
              ...
              <div className="p-6">
                <div className="relative">
                  {timeline.steps && timeline.steps.map((step, idx) => (
