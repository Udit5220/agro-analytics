...
import { TrendingUp, LineChart, Columns, BarChart3, MapPin, Landmark, ArrowUpRight, Award } from "lucide-react";

export default function MarketIntelligenceDashboard() {
  const metrics = [
    { label: "Crops Tracked", value: "102", sub: "National scale", color: "text-[#31572c] bg-[#31572c]/10" },
    { label: "Top Mandi Gain", value: "Mustard", sub: "+3.2% this week", color: "text-emerald-700 bg-emerald-50" },
    { label: "National Volume", value: "1.2M Tons", sub: "Active exchanges", color: "text-sky-700 bg-sky-50" },
    { label: "Mandi Listings", value: "248", sub: "Haryana region", color: "text-amber-705 bg-amber-50" },
  ];

  const mandiPrices = [
    { mandi: "Faridabad APMC", crop: "Wheat (गेहूं)", price: "₹2,440", change: "+2.1%", volume: "450 Tons" },
    { mandi: "Palwal Grain Market", crop: "Rice (धान)", price: "₹2,420", change: "+1.8%", volume: "380 Tons" },
    { mandi: "Ballabhgarh Market", crop: "Mustard (सरसों)", price: "₹5,410", change: "+3.2%", volume: "120 Tons" },
    { mandi: "Hodal Mandi", crop: "Maize (मक्का)", price: "₹2,090", change: "+0.9%", volume: "290 Tons" },
  ];
              ...
              <tbody className="divide-y divide-gray-100/60">
                {mandiPrices.map((item, idx) => (
                  <tr key={idx} className="hover:bg-[#f4f7f4]/30 transition-colors text-xs font-semibold">
                    <td className="p-3 pl-1 text-gray-900 font-bold">{item.mandi}</td>
                    <td className="p-3 text-gray-550">{item.crop}</td>
                    <td className="p-3 text-emerald-700 font-black">{item.price}</td>
                    <td className="p-3 text-right pr-2 text-emerald-700 font-bold">{item.change}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-200/60 shadow-sm space-y-4">
          ...
          <div className="space-y-3">
            <div className="bg-emerald-50/50 border border-emerald-100 p-3 rounded-xl flex gap-2.5">
              <ArrowUpRight size={18} className="text-[#31572c] shrink-0 mt-0.5" />
              <div>
                <span className="text-[10px] font-bold text-[#132a13] block">High Demand Alerts</span>
                <span className="text-[11px] text-gray-600 block mt-0.5 leading-relaxed font-semibold">
                  Mustard crop matches high volume benchmarks; sell-out returns expected.
                </span>
              </div>
            </div>
            ...
            <div className="bg-[#f4f7f4] border border-gray-150 p-3 rounded-xl flex gap-2.5">
              ...
              <div>
                <span className="text-[10px] font-bold text-gray-800 block">MSP Price Guard</span>
                <span className="text-[11px] text-gray-550 block mt-0.5 leading-relaxed font-medium">
                  Haryana state MSP guarantees minimum ₹2,275 per Quintal Wheat support levels.
                </span>
              </div>
            </div>
          </div>
