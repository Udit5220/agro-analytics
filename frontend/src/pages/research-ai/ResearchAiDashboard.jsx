...
import { Leaf, Search, FileText, Globe, Brain, Sparkles, Plus, ArrowRight } from "lucide-react";

export default function ResearchAiDashboard() {
  const metrics = [
    { label: "Indexed Papers", value: "450+", sub: "Scientific database", color: "text-[#31572c] bg-[#31572c]/10" },
    { label: "PDF Size Limit", value: "50 MB", sub: "Max storage capacity", color: "text-emerald-700 bg-emerald-50" },
    { label: "Translation Nodes", value: "12 Languages", sub: "Bilingual engines", color: "text-sky-700 bg-sky-50" },
    { label: "RAG Search Logs", value: "48 queries", sub: "Optimized response", color: "text-amber-700 bg-amber-50" },
  ];

  const recentPapers = [
    { title: "Optimal Nitrogen Blends for Wheat Yields in Semi-Arid Soil", date: "May 2026", size: "4.2 MB", status: "Indexed" },
    { title: "Pathological Identification of Leaf Rust (Puccinia triticina) via CNNs", date: "April 2026", size: "8.1 MB", status: "Indexed" },
    { title: "Water Stagnation and Root Decay Tolerances in Oryza Sativa", date: "March 2026", size: "3.5 MB", status: "Indexed" },
  ];
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-200/60 shadow-sm overflow-hidden flex flex-col h-[400px]">
          ...
          <div className="bg-[#f4f7f4] border-b border-gray-100 p-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-black text-gray-800 uppercase tracking-widest">Document RAG Engine</span>
            </div>
            <span className="text-[10px] font-bold text-[#31572c]">GPT-4o Deep Semantics</span>
          </div>

          {/* RAG Context Window */}
          <div className="flex-1 p-4 overflow-y-auto space-y-3.5 text-xs">
            <div className="flex gap-2.5">
              <div className="h-7 w-7 rounded-lg bg-[#31572c]/10 text-[#31572c] flex items-center justify-center shrink-0 font-bold text-[10px]">RAG</div>
              <div className="bg-[#f4f7f4] border border-gray-100 rounded-xl p-3 max-w-[85%]">
                <p className="font-bold text-gray-900 mb-1">RAG Retrieval Core Initialized</p>
                <p className="text-gray-650 leading-relaxed font-medium">
                  Welcome to AgroIndia Pathology & Agronomic Research RAG. Upload any PDF agronomy circular or search our national directory. Ask me complex biological questions below.
                </p>
              </div>
            </div>

            <div className="flex gap-2.5 flex-row-reverse">
              <div className="h-7 w-7 rounded-lg bg-[#132a13] text-white flex items-center justify-center shrink-0 font-bold text-[10px]">USER</div>
              <div className="bg-[#31572c] text-white rounded-xl p-3 max-w-[80%]">
                <p className="leading-relaxed font-semibold">
                  Summarize recommended treatments for Rice Bacterial Leaf Blight based on Indian Council of Agricultural Research guidelines.
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-2.5">
              ...
              <div className="bg-[#f4f7f4] border border-gray-100 rounded-xl p-3 max-w-[85%]">
                <span className="inline-block text-[9px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded mb-2">
                  Retrieved 2 documents (ICAR Bulletin 2024; Pathology Circular 18)
                </span>
                <p className="text-gray-650 leading-relaxed font-medium">
                  According to the retrieved bulletins: <br />
                  1. Spray Agrimycin-100 (0.05%) paired with Copper Oxychloride (0.3%) at first onset. <br />
                  2. Avoid excess Nitrogen application (limit top dressing during humid weeks). <br />
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-200/60 shadow-sm space-y-4">
          ...
          <div className="space-y-3">
            {recentPapers.map((paper, idx) => (
              <div key={idx} className="bg-[#f4f7f4] hover:bg-[#31572c]/5 border border-gray-150 p-3 rounded-xl flex flex-col justify-between hover:shadow-sm cursor-pointer transition-all">
                <span className="text-[11px] font-bold text-gray-900 line-clamp-2 leading-tight">
                  {paper.title}
                </span>
                <div className="flex items-center justify-between mt-2.5">
                  <span className="text-[9px] font-bold text-gray-400">{paper.date} • {paper.size}</span>
