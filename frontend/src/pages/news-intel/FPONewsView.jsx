...
import { ChevronDown, ChevronUp, AlertTriangle, TrendingUp, Users, Package, Truck, FileCheck, BarChart3, MapPin, Clock, ShieldAlert, Loader2, Info as LucideInfo } from 'lucide-react';
import { BarChart, Bar, LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from 'recharts';

// Reusable Accordion Item Component (Indigo Theme)
...
// Global Page Chart Gallery Component (Indigo Theme)
      ...
      {charts.map((chart, idx) => (
        <div key={idx} className="bg-white border border-emerald-100 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow">
          <h4 className="text-sm font-bold text-emerald-700 uppercase tracking-wider mb-4 flex items-center gap-2">
            ...
            {chart.title || "Institutional Analytics"}
          </h4>
...
export default function FPONewsView({ data, subPath }) {
    ...
    return (
      <div className="w-full pb-20">
        <div className="text-center py-16 text-emerald-500 bg-emerald-50 rounded-2xl border border-emerald-200 border-dashed">
          ...
          <p className="text-sm font-medium">{t("Aggregating Institutional Intelligence...")}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full pb-20 animate-fadeIn">
      ...
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-sm font-bold text-emerald-800 uppercase tracking-wider">{t("FPO Operations Feed")}</h3>
