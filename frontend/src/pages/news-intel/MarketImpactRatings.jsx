    ...
    <div className="space-y-6 animate-fadeIn antialiased">
      ...
      <div className="bg-white border border-emerald-100 rounded-3xl p-6 shadow-xs relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between">
        <div className="flex items-start space-x-4 z-10">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-bold tracking-tight text-emerald-900">Market Impact & AI Ratings</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-emerald-100 rounded-3xl p-6 shadow-sm flex flex-col items-center justify-between min-h-[320px]">
          <div className="w-full">
            <span className="text-[10px] font-black text-emerald-400 tracking-wider uppercase block text-center mb-4">
          ...
          <div className="relative flex flex-col items-center justify-center">
            ...
            <div className="w-32 h-32 relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-3xl font-extrabold text-emerald-900 tracking-tight">{sentimentScore}</span>
          ...
          <p className="text-xs text-emerald-500 text-center leading-relaxed mt-4 max-w-xs font-medium">
            AI analysis of live crawlers and news indices outputs this real-time sentiment index.
          </p>
        </div>

        {/* Card 2: High Volatility Alert */}
        <div className="bg-emerald-50/70 border border-emerald-100 rounded-3xl p-6 shadow-sm flex flex-col justify-between min-h-[320px] relative overflow-hidden">
          <div className="absolute -right-4 -bottom-4 opacity-5 pointer-events-none">
            <AlertOctagon className="w-40 h-40 text-emerald-950" />
          </div>
          ...
          <div>
            ...
            <h3 className="text-2xl font-black text-emerald-950 tracking-tight">{activeCommodityName}</h3>
            <p className="text-xs font-extrabold text-emerald-800 mt-1">Risk Index: {volatilityScore}/100</p>
          </div>

          <div className="space-y-3 mt-4 relative z-10">
            {/* Driver Box */}
            <div className="bg-white p-3 rounded-xl border border-emerald-100/50 shadow-2xs">
              <p className="text-xs text-emerald-950 leading-relaxed font-semibold">
                <span className="text-[10px] font-black text-emerald-600 block tracking-wide mb-0.5">DRIVER:</span>
                {driverText}
              </p>
            </div>
            ...
            <div className="bg-white p-3 rounded-xl border border-emerald-100/50 shadow-2xs">
              <p className="text-xs text-emerald-950 leading-relaxed font-semibold">
                <span className="text-[10px] font-black text-emerald-600 block tracking-wide mb-0.5">ADVISED ACTION:</span>
                {actionText}
              </p>
            </div>
          </div>
        ...
        <div className="bg-white border border-emerald-100 rounded-3xl p-6 shadow-sm flex flex-col justify-between min-h-[320px] relative">
          <div>
            <span className="text-[10px] font-black text-emerald-400 tracking-wider uppercase block mb-5">
            ...
            <div className="space-y-4 relative" onMouseMove={handleMouseMove}>
              {commodities.map((crop) => (
                ...
                >
                  <div className="flex items-center justify-between text-xs font-bold text-emerald-700">
                    ...
                    <span className="text-emerald-400 font-mono text-[10px]">{crop.percentage}% Impact</span>
                  </div>
              ...
              {hoveredCrop && (
                <div 
                  className="absolute bg-emerald-900 text-white rounded-lg p-2.5 text-[10px] shadow-lg pointer-events-none z-20 max-w-[200px] border border-emerald-800/80 animate-fadeIn space-y-1 font-sans"
                  style={{ left: `${tooltipPos.x}px`, top: `${tooltipPos.y}px` }}
                >
                  <div className="font-extrabold text-emerald-400 border-b border-emerald-800 pb-1 mb-1">
...
function CustomChartTooltip({ active, payload, label }) {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-emerald-900 border border-emerald-800 text-white p-3.5 rounded-xl shadow-xl space-y-2 max-w-[220px] font-sans">
        <div className="text-[10px] font-black text-emerald-400 uppercase tracking-widest border-b border-emerald-850 pb-1">
          {label} Analysis
        </div>
        ...
        <div className="text-[9px] text-emerald-350 leading-relaxed font-semibold bg-emerald-950/80 p-1.5 rounded border border-emerald-800/40">
          {sentiment < 30 ? (
            <span className="text-emerald-450 flex items-center gap-1">
              <AlertOctagon className="w-3 h-3 text-emerald-500 shrink-0" />
