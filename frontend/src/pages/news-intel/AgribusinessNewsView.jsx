...
import { ChevronDown, ChevronUp, AlertTriangle, TrendingUp, Crosshair, BarChart3, Truck, ShoppingCart, Briefcase, Building, Clock, ArrowRight, Loader2, Info as LucideInfo } from 'lucide-react';
import { BarChart, Bar, LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from 'recharts';

// Reusable Accordion Item Component (Amber/Orange Theme)
...
// Global Page Chart Gallery Component (Amber/Orange Theme)
      ...
      {charts.map((chart, idx) => (
        <div key={idx} className="bg-white border border-emerald-100 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow">
          <h4 className="text-sm font-bold text-emerald-700 uppercase tracking-wider mb-4 flex items-center gap-2">
            ...
            {chart.title || "Corporate Analytics"}
          </h4>
              ...
              {chart.type === 'bar' ? (
                <BarChart data={chart.data} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
                  ...
                  <Tooltip 
                    ...
                    itemStyle={{ color: '#c2410c', fontWeight: 'bold' }}
                  />
                ...
                <LineChart data={chart.data} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 10, fill: '#10b981' }} tickLine={false} axisLine={false} />
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#ffffff', borderColor: '#047857', borderRadius: '8px', fontSize: '12px' }}
                    itemStyle={{ color: '#9a3412', fontWeight: 'bold' }}
                  />
...
export default function AgribusinessNewsView({ data, subPath }) {
    ...
    return (
      <div className="w-full pb-20">
        <div className="text-center py-16 text-emerald-500 bg-emerald-50 rounded-2xl border border-emerald-200 border-dashed">
          ...
          <p className="text-sm font-medium">{t("Aggregating Corporate Intelligence...")}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full pb-20 animate-fadeIn">
      ...
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-sm font-bold text-emerald-800 uppercase tracking-wider">{t("Enterprise Intelligence synthesis")}</h3>
