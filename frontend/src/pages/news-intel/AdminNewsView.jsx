...
import { ChevronDown, ChevronUp, Database, Globe, Activity, Users, Shield, Terminal, Clock, Settings, ArrowRight, Loader2, Info as LucideInfo } from 'lucide-react';
import { BarChart, Bar, LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from 'recharts';

// Reusable Accordion Item Component (Slate/Zinc Theme)
...
// Global Page Chart Gallery Component (Slate/Zinc Theme)
  ...
  return (
    <div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-4">
      {charts.map((chart, idx) => (
        <div key={idx} className="bg-white border border-emerald-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow">
            ...
            <ResponsiveContainer width="100%" height="100%">
              {chart.type === 'bar' ? (
                <BarChart data={chart.data} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
                  ...
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#ffffff', borderColor: '#059669', borderRadius: '8px', fontSize: '12px' }}
...
export default function AdminNewsView({ data, subPath }) {
    ...
    return (
      <div className="w-full pb-20">
        <div className="text-center py-16 text-emerald-500 bg-emerald-50 rounded-2xl border border-emerald-200 border-dashed">
          <Terminal className="w-8 h-8 mx-auto mb-3 text-emerald-400" />
          <p className="text-sm font-medium font-mono">{t("Connecting to Infrastructure Telemetry...")}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full pb-20 animate-fadeIn">
      ...
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-sm font-bold text-emerald-800 uppercase tracking-wider">{t("System Infrastructure Telemetry")}</h3>
        <span className="text-xs font-bold text-emerald-700 bg-emerald-200 px-2 py-1 rounded-md font-mono">
